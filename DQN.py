...
class DQN(nn.Module):
    def __init__(self, img_height, img_width):
        super().__init__()
        ##############################################TO DO###############################################
        ## Put the correct input features (hint: your input image is rgb and may have several channels) ##
        self.fc1 = nn.Linear(in_features=None,out_features=24)
        ##                                                                                              ##
        ##################################################################################################
...
class Agent():
    ...
    def select_action(self, state, policy_net):
        ...
        if rate > random.random():
            return torch.from_numpy(np.array([random.randrange(self.num_actions)])).to(self.device) #explore
        else:
            with torch.no_grad():
...
class CartPoleEnvManager():
    def __init__(self, device):
        self.device = device 
        self.env = gym.make('CartPole-v1').env.unwrapped
        self.env.reset()
...
class QValues():
    ...
    def get_next(None, next_states):
        ...
        values[non_final_state_locations] = None(non_final_states).max(dim=1)[0].detach()
        return values
    ##########################################################################
...
target_net.eval() #network only uses for inference
...
def final_plot(moving_avg_period, values):
    plt.figure(2)
...
final_plot(100, episode_durations)